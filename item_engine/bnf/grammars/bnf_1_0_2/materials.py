...
__all__ = ['Any_', 'All_', 'CharsetArg', 'GrammarRule', 'Atom_', 'Str', 'Var', 'Match', 'MatchAs', 'MatchIn', 'All', 'Any', 'Optional', 'Repeat', 'Enum', 'Charset', 'Operator', 'Group', 'Grammar', 'build']
...
class Grammar:
    def __init__(self, version: str, branches: List[GrammarRule]):
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}({self.version!r}, {self.branches!r})'
    
    def __eq__(self, other):
        if type(self) is type(other):
            return self.version == other.version and self.branches == other.branches
        else:
            return NotImplemented
...
def build(obj):
    if isinstance(obj, Lemma):
        if obj.value == 'Str':
            ...
            return Grammar(version=build(obj.data['version']), branches=list(map(build, obj.data['branches'])))
        else:
