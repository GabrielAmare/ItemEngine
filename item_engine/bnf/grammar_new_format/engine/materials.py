...
class Grammar:
    def __init__(self, version: str, args: List[GrammarArg]):
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}({self.version!r}, {self.args!r})'
    
    def __eq__(self, other):
        if type(self) is type(other):
            return self.version == other.version and self.args == other.args
        else:
            return NotImplemented
...
def build(obj):
    if isinstance(obj, Lemma):
        if obj.value == 'Str':
            ...
            return Grammar(version=build(obj.data['version']), args=list(map(build, obj.data['args'])))
        else:
