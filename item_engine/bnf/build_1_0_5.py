...
from item_engine.bnf.utils import ClassBuild
...
class GrammarWrapper:
    def __init__(self, ctx: EngineBuilder, obj: gr.Grammar):
        self.ctx: EngineBuilder = ctx
        self.obj: gr.Grammar = obj

        self.charsets: List[gr.Charset] = []
        self.patterns: List[gr.Pattern] = []
    ...
    def _register_pattern(self, obj: gr.Pattern):
...
class OperatorWrapper:
    def __init__(self, ctx: EngineBuilder, obj: gr.Operator):
        self.ctx: EngineBuilder = ctx
        self.obj: gr.Operator = obj


class PatternWrapper:
    def __init__(self, ctx: EngineBuilder, obj: gr.Pattern):
        self.ctx: EngineBuilder = ctx
        self.obj: gr.Pattern = obj


class GroupWrapper:
    def __init__(self, ctx: EngineBuilder, obj: gr.Group):
        self.ctx: EngineBuilder = ctx
        self.obj: gr.Group = obj


class CharsetWrapper:
    def __init__(self, ctx: EngineBuilder, obj: gr.Charset):
        self.ctx: EngineBuilder = ctx
...
class EngineBuilder:
    ...
    def _get_literal_expr(self, literal: gr.Str) -> str:
        expr = eval(literal.expr).strip(self.whitespace)
        if expr:
            return expr
        else:
    ...
    def _list_all(self, grammar: gr.Grammar) -> Tuple[List[gr.Group], List[gr.Operator]]:
        grammar_wrapper: GrammarWrapper = GrammarWrapper(ctx=self, obj=grammar)

        for charset in grammar_wrapper.charsets:
            print(charset)
            pass

        for pattern in grammar_wrapper.patterns:
            print(pattern)
            pass

        for operator in grammar_wrapper.operators:
            print(operator)
            self.builds[operator.name] = OperatorBuild(context=self, operator=operator)

        for group in grammar_wrapper.groups:
            print(group)
            self.builds[group.name] = GroupBuild(context=self, group=group)
            self.type_aliases[group.name] = group.names

        self._build_groups(grammar_wrapper.groups)

        return grammar_wrapper.groups, grammar_wrapper.operators

    def _build_groups(self, groups: List[gr.Group]):
        for group in groups:
    ...
    @property
    def _max_sup_level(self) -> int:
        return max([build.sup_level for build in self.builds.values()], default=0)

    def _get_lemmatizer_branchset(self, operators: List[gr.Operator]) -> Union[BranchSet, Branch]:
        branches = []
        for operator in operators:
            build: OperatorBuild = self.builds[operator.name]
            branches.append(
                Branch(
                    name=operator.name,
                    rule=self._get_lemmatizer_rule(operator.rule),
                    priority=1# + build.sup_level
                )
            )
    ...
    def _generate_build_function(self) -> DEF:
        ...
        operator_builds = [
            build
            for build in sorted(self.builds.values(), key=lambda build: -build.sub_level)
    ...
    def generate_engine(self, grammar: gr.Grammar, patterns: List[Branch] = None,
        ...
        classes = [build.get_class() for build in sorted(self.builds.values(), key=lambda build: -build.sub_level)]
