...
class Context(MR):
    ...
    @MR.wrap
    def _str(self, obj: gr.MatchAs) -> EXPRESSION:
        return VAR("str").CALL(SELF.GETATTR(obj.key))

    @MR.wrap
    def _str(self, obj: gr.MatchIn) -> EXPRESSION:
        return VAR("map").CALL(VAR("str"), SELF.GETATTR(obj.key))

    @MR.wrap
    def _str(self, obj: gr.All) -> EXPRESSION:
    ...
    @MR.wrap
    def _str(self, obj: gr.Enum) -> EXPRESSION:
    ...
    @MR.wrap
    def _str(self, obj: gr.EnumP) -> EXPRESSION:
        sep = self._str(obj.separator)
        if isinstance(sep, OBJECT):
            return sep.METH("join", self._str(obj.child))
        else:
    ...
    ####################################################################################################################
    @MR.wrap
    def var_dict(self, obj: gr.MatchAs) -> Dict[str, Tuple[str, bool]]:
        return {obj.key: (obj.name, False)}

    @MR.wrap
    def var_dict(self, obj: gr.MatchIn) -> Dict[str, Tuple[str, bool]]:
        return {obj.key: (obj.name, True)}

    @MR.wrap
    def var_dict(self, obj: gr.All) -> Dict[str, Tuple[str, bool]]:
    ...
    @MR.wrap
    def var_dict(self, obj: gr.Any) -> Dict[str, Tuple[str, bool]]:
        first, *args = obj.args
    ...
    @MR.wrap
    def var_dict(self, obj: gr.Enum) -> Dict[str, Tuple[str, bool]]:
        return self.var_dict(obj.child)

    @MR.wrap
    def var_dict(self, obj: gr.EnumP) -> Dict[str, Tuple[str, bool]]:
        return self.var_dict(obj.child)
    ...
    @MR.wrap
    def var_dict(self, obj: gr.Any_) -> Dict[str, Tuple[str, bool]]:
        return {}
    ...
    @MR.wrap
    def _tokenizer_rule(self, obj: gr.P_Any) -> ie.Rule:
    ...
    @MR.wrap
    def _tokenizer_rule(self, obj: gr.P_Any_) -> ie.Rule:
        raise NotImplementedError
    ...
    @MR.wrap
    def _tokenizer_branches(self, obj: gr.All) -> Iterator[Branch]:
        for arg in obj.args:
            yield from self._tokenizer_branches(arg)

    @MR.wrap
    def _tokenizer_branches(self, obj: gr.Any) -> Iterator[Branch]:
        for arg in obj.args:
            yield from self._tokenizer_branches(arg)

    @MR.wrap
    def _tokenizer_branches(self, obj: gr.Enum) -> Iterator[Branch]:
        yield from self._tokenizer_branches(obj.separator)

    @MR.wrap
    def _tokenizer_branches(self, obj: gr.EnumP) -> Iterator[Branch]:
        yield from self._tokenizer_branches(obj.separator)
    ...
    def _tokenizer_branches(self, obj: gr.Pattern) -> Iterator[Branch]:
        yield ie.Branch(
            name=obj.name,
            rule=self._tokenizer_rule(obj.arg),
            priority=0
        )

    @MR.wrap
    def _tokenizer_branches(self, obj: gr.Any_) -> Iterator[Branch]:
        return []
