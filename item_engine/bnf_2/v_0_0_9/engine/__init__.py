...
def parse_characters(src: str) -> Iterator[Char]:
    pos: int = -1
...
def parse_toplevel(src: Iterator[Lemma]) -> Lemma:
    res: List[Lemma] = []
    to: int = 0
    eof: int = 0
    for inp in src:
...
def parse(src: str) -> Lemma:
    return parse_toplevel(lemmatizer(tokenizer(parse_characters(src))))
