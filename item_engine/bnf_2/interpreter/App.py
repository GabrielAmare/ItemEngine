...
class App(Tk):
    ...
    def __init__(self, characters, tokenizer, lemmatizer, transpiler, style, input_file=None):
        ...
        self.view.bind('<Any-KeyRelease>', self.on_key_release, add=True)
    ...
    def check_timer(self, __DELAY=1):
        now = time.time()

        remain = __DELAY - (now - self.start_timer)

        if remain <= 0:
            self.analyse()
            self.lock_timer = False
        else:
            ms = max(int(remain * 1000), 333)
            self.after(ms, self.check_timer)

    def on_key_release(self, _evt):
        now = time.time()
    ...
    def analyse(self):
        self.after(0, self.analyse_parallel)
